...
const buttonVariants = cva(
  "clip-frame liquid-glass relative isolate inline-flex items-center justify-center gap-2 overflow-hidden whitespace-nowrap font-semibold transition-[background,transform,color] duration-300 ease-soft outline-none focus-visible:ring-2 focus-visible:ring-brand-400/60 disabled:pointer-events-none disabled:opacity-45 active:scale-[0.98] [&_svg]:shrink-0",
  {
    variants: {
      variant: {
        primary:
          "bg-[var(--acid)] text-[var(--ink)] hover:bg-brand-50",
        money:
          "bg-[var(--acid)] text-[var(--ink)] hover:bg-brand-50",
        secondary:
        ...
        support:
          "bg-support text-[#0d1a20] hover:bg-[#9ae0ff]",
      },
      size: {
        sm: "h-9 rounded-xl px-3 text-[13px] [--clip-radius:0.75rem] [&_svg]:size-4",
        md: "h-11 rounded-2xl px-4 text-sm [--clip-radius:1rem] [&_svg]:size-4",
        lg: "h-13 rounded-2xl px-5 text-[15px] [--clip-radius:1rem] [&_svg]:size-5",
        icon: "size-10 rounded-xl [--clip-radius:0.75rem] [&_svg]:size-4",
        pill: "h-8 rounded-pill px-3 text-[13px] [--clip-radius:999px] [&_svg]:size-3.5",
      },
